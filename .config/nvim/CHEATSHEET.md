...
The left sidebar runs a **nested tmux** (its own prefix `Ctrl-a`) so you can keep several Claude instances as tabs in that one pane. Each ide window has its own independent set. The tab strip shows along the top of the sidebar — **click a tab** to switch, or use the keys below (focus must be in the sidebar pane):

| Keys | Action |
|------|--------|
| `C-a c` | new **Claude** tab |
| `C-a C` | new plain **shell** tab |
| `C-a n` / `C-a p` | next / previous tab |
| `C-a 1`…`9` | jump to tab N |
| `C-a C-a` | last (previous) tab |
| `C-a ,` | rename the current tab |
| `C-a &` | close the current tab |

These tabs live on a separate tmux server that **survives `ide kill`**. To wipe every Claude tab everywhere: `tmux -L ide-claude kill-server`.
...
| `Space b o` | close **all other** files (keep this one) |
| `Space b D` | close **all** files in the group |
...
| `Space c i` | toggle **inlay hints** (inline parameter names + deduced types) |
